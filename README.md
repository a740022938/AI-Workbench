...
We provide `config.example.json` with default settings. For first-time setup:
...
2. Or create `config.json` manually with the following structure:
```json
{
    "paths": {
        "image_dir": "./images",
...
### Installation Guides
- **[Windows 11 Installation Guide](docs/WIN11_INSTALL_GUIDE.md)** - Complete step-by-step installation for Windows 11 users
- **[Quick Start for Windows 11](docs/QUICK_START_WIN11.md)** - Fastest path to launch for users with Python already installed
- **[Troubleshooting Guide](docs/TROUBLESHOOTING.md)** - Solutions for common installation and runtime issues

### For Different User Types
- **Beginners**: Start with the Windows 11 Installation Guide
- **Developers**: Use the Quick Start guide and refer to code documentation
- **AI/ML Practitioners**: Focus on the Training Center and Dataset Center features

### Important Notes for Windows Users
- This is a **Technical Preview** - Expect some rough edges in installation
- Python 3.8+ is required, with proper PATH configuration
...
- **Symptoms**: `ModuleNotFoundError: No module named 'PIL'` or `ImportError: cannot import name 'Image'`
- **Solution**: Ensure Pillow is installed correctly: `pip install --upgrade Pillow`
- **Note**: Some systems may have `PIL` installed instead of `Pillow`; Pillow is the modern fork

#### GUI Launch Failures
- **Symptoms**: Application crashes on startup or window fails to appear
- **Check**: Verify all dependencies are installed: `pip install -r requirements.txt`
- **Check**: Ensure Python version is 3.8 or higher: `python --version`
- **Check**: For Linux systems, ensure tkinter is installed: `sudo apt-get install python3-tk`

#### Module Import Errors
- **Symptoms**: `ImportError: cannot import name '...'` for project modules
- **Solution**: Ensure you're running from the project root directory
- **Solution**: Add project root to Python path: `export PYTHONPATH=/path/to/AI-Workbench:$PYTHONPATH`

#### CUDA/GPU Acceleration Issues
- **Symptoms**: Training runs slowly or PyTorch cannot detect GPU
- **Check**: Install PyTorch with CUDA support (see requirements.txt notes)
- **Check**: Verify CUDA drivers are installed: `nvidia-smi`
- **Fallback**: Use CPU-only mode by setting `device: "cpu"` in training configuration

#### Configuration File Issues
- **Symptoms**: Paths not working or configuration not loading
- **Solution**: Copy `config.json` to a new location if needed, or edit paths to match your system
- **Note**: All paths in `config.json` are relative to the project root by default
...
- **Documentation**: [https://docs.ai-workbench.dev](https://docs.ai-workbench.dev) (planned)
- **Discord Community**: [https://discord.gg/ai-workbench](https://discord.gg/ai-workbench) (planned)
