...
from ui.data_health_window import open_data_health_window
from ui.dataset_export_window import open_dataset_export_window
from ui.closed_loop_window import open_closed_loop_window
from core.action_system import get_global_dispatcher, execute_action_unified, get_state_snapshot
...
class MainWindow:
    def __init__(self, root):
        self.root = root
        self._training_monitor_window = None
        ...
        # 兼容旧版样式
        self.ui_style = self._get_compatible_style()
        ...
        # 暂时保留部分旧字段
        self.valid_ext = self.context.valid_ext
        ...
        # ==================== 主板插槽预留 ====================
        # 未来组织架构插槽：负责模块间协调和职责分配
        self.organization = None
        
        # 未来调度器插槽：负责任务调度和流程控制
        self.dispatcher = None
        
        # 未来助手角色插槽：负责特定领域的辅助功能
        self.assistant_roles = []
        
        # 未来报告器插槽：负责状态监控和汇报
        self.reporter = None
        
        # 未来发布管理器插槽：负责版本控制和稳定发布
        self.release_manager = None
        # ==================== 主板插槽预留结束 ====================
        
        # ==================== 老板背包 ====================
        # 轻量级背包系统，用于存储工具、资源和运行时数据
        self.backpack = {
            'tools': {},           # 工具集合（未来可扩展）
            'resources': {},       # 资源缓存（如图片缓存、模型引用等）
            'notes': [],           # 临时笔记和提醒
            'runtime_data': {      # 运行时状态数据
    ...
    def _get_compatible_style(self):
        """获取兼容旧版样式的字典"""
        style_mgr = self.style_manager
        theme = style_mgr.get_current_theme_definition()
        
        # 构建兼容字典（与get_basic_style返回的格式类似）
        return {
            # 颜色
            "app_bg": theme.colors.get("background", "#16181d"),
            "panel_bg": theme.colors.get("surface", "#252a33"),
            "text_fg": theme.colors.get("text_primary", "#f5f7fa"),
            "muted_fg": theme.colors.get("text_secondary", "#aeb6c2"),
            "accent": theme.colors.get("primary", "#4da3ff"),
            "success": theme.colors.get("success", "#67d39a"),
            "warning": theme.colors.get("warning", "#ff9d57"),
            "error": theme.colors.get("error", "#ff6b6b"),
            ...
            # 间距
            "padding_x": style_mgr.get_spacing("md"),
            "padding_y": style_mgr.get_spacing("sm"),
            "button_width": 12,
            
            # 透明度
            "opacity": self.style_manager.current_opacity,
        }
    
    def _refresh_ui_on_language_change(self):
        """语言切换时刷新UI文案"""
        # 更新窗口标题
        self.root.title(t("TITLE_MAIN_WINDOW"))
        
        # 更新有引用的按钮文本
        if hasattr(self, 'ai_analysis_btn'):
            self.ai_analysis_btn.config(text=get_text("BTN_AI_ANALYSIS"))
        if hasattr(self, 'training_monitor_btn'):
            self.training_monitor_btn.config(text=t("TOOL_TRAINING_MONITOR"))
        if hasattr(self, 'training_center_btn'):
            self.training_center_btn.config(text=t("TOOL_TRAINING_CENTER"))
        if hasattr(self, 'import_training_config_btn'):
            self.import_training_config_btn.config(text=t("TOOL_IMPORT_TRAINING_CONFIG"))
        if hasattr(self, 'import_class_config_btn'):
            self.import_class_config_btn.config(text=t("TOOL_IMPORT_CLASS_CONFIG"))
        if hasattr(self, 'data_health_check_btn'):
            self.data_health_check_btn.config(text=t("TOOL_DATA_HEALTH_CHECK"))
        if hasattr(self, 'closed_loop_btn'):
            self.closed_loop_btn.config(text=t("TOOL_CLOSED_LOOP"))
        
        # 更新主窗口匿名按钮（现在已有引用）
        if hasattr(self, 'open_project_btn'):
            self.open_project_btn.config(text=get_text("BTN_OPEN_PROJECT"))
        if hasattr(self, 'save_btn'):
            self.save_btn.config(text=get_text("BTN_SAVE"))
        if hasattr(self, 'auto_detect_btn'):
            self.auto_detect_btn.config(text=get_text("BTN_AUTO_DETECT"))
        if hasattr(self, 'export_btn'):
            self.export_btn.config(text=get_text("BTN_EXPORT"))
        if hasattr(self, 'fit_window_btn'):
            self.fit_window_btn.config(text=get_text("BTN_FIT_WINDOW"))
        if hasattr(self, 'original_scale_btn'):
            self.original_scale_btn.config(text=get_text("BTN_ORIGINAL_SCALE"))
        if hasattr(self, 'settings_btn'):
            self.settings_btn.config(text=get_text("BTN_SETTINGS"))
        
        # 更新标签文本
        if hasattr(self, 'image_info_label'):
            self.image_info_label.config(text=get_text("STATUS_WAITING_LOAD"))
        
        if hasattr(self, 'project_info_label'):
            self.project_info_label.config(text=get_text("STATUS_WAITING_PROJECT_INFO"))
        
        # 刷新画布和控制面板（如果存在）
        if hasattr(self, 'canvas_panel') and self.canvas_panel:
            # 画布面板可能需要刷新内部文案
            pass
        ...
        # 记录日志
        print(f"UI文案已刷新，当前语言: {self.language_manager.current_language.value}")
    ...
    def _refresh_ui_on_style_change(self):
        ...
        # 刷新主窗口背景
        self.root.config(bg=self.ui_style["app_bg"])
        
        # 刷新顶部按钮区背景
        if hasattr(self, 'top_frame'):
            self.top_frame.config(bg=self.ui_style["app_bg"])
        
        # 刷新左侧画布区背景
        if hasattr(self, 'left_frame'):
            self.left_frame.config(bg=self.ui_style["app_bg"])
        
        # 刷新右侧控制面板背景
        if hasattr(self, 'right_panel_frame'):
            self.right_panel_frame.config(bg=self.ui_style["panel_bg"])
        
        # 刷新画布面板（如果存在）
        if hasattr(self, 'canvas_panel') and self.canvas_panel:
            # 画布面板有自己的刷新逻辑，暂时只更新背景
            pass
        
        # 刷新右侧面板（如果存在）
        if hasattr(self, 'right_panel') and self.right_panel:
            # 右侧面板有自己的刷新逻辑，暂时只更新背景
            pass
        
        # 刷新按钮颜色（主要按钮）
        button_widgets = [
            'open_project_btn', 'save_btn', 'auto_detect_btn', 'export_btn',
            'ai_analysis_btn', 'training_monitor_btn', 'training_center_btn',
        ...
        for btn_name in button_widgets:
            if hasattr(self, btn_name):
                ...
                try:
                    btn.config(bg=self.ui_style.get("accent", "#4da3ff"), 
                              fg=self.ui_style.get("text_fg", "#f5f7fa"))
                except:
                    pass
        
        # 刷新标签颜色
        label_widgets = [
            'image_info_label', 'project_info_label'
        ]
        ...
        for label_name in label_widgets:
            if hasattr(self, label_name):
                ...
                try:
                    label.config(bg=self.ui_style.get("app_bg", "#16181d"),
                                fg=self.ui_style.get("text_fg", "#f5f7fa"))
                except:
                    pass
        
        # 记录日志
        print(f"UI风格已刷新，当前风格: {self.style_manager.current_style.value}")

    # ==================== 兼容属性层 ====================
    # 旧代码通过 self.xxx 访问，实际映射到 self.context.xxx
    
    @property
    def current_class_names(self):
        return self.context.current_class_names
    
    @current_class_names.setter
    def current_class_names(self, value):
        self.context.current_class_names = value
    
    @property
    def class_source_name(self):
        return self.context.class_source_name
    
    @class_source_name.setter
    def class_source_name(self, value):
        self.context.class_source_name = value
    
    @property
    def image_dir(self):
        return self.context.image_dir
    
    @image_dir.setter
    def image_dir(self, value):
        self.context.image_dir = value
    
    @property
    def label_dir(self):
        return self.context.label_dir
    
    @label_dir.setter
    def label_dir(self, value):
        self.context.label_dir = value
    
    @property
    def auto_save(self):
        return self.context.auto_save
    
    @auto_save.setter
    def auto_save(self, value):
        self.context.auto_save = value
    
    @property
    def image_files(self):
        return self.context.image_files
    
    @image_files.setter
    def image_files(self, value):
        self.context.image_files = value
    ...
    def current_index(self, value):
        self.context.current_index = value
    
    @property
    def original_image(self):
        return self.context.original_image
    
    @original_image.setter
    def original_image(self, value):
        self.context.original_image = value
    ...
    def display_image(self, value):
        self.context.display_image = value
    
    @property
    def current_tk(self):
        return self.context.current_tk
    
    @current_tk.setter
    def current_tk(self, value):
        self.context.current_tk = value
    
    @property
    def current_image_name(self):
        return self.context.current_image_name
    
    @current_image_name.setter
    def current_image_name(self, value):
        self.context.current_image_name = value
    
    @property
    def img_offset_x(self):
        return self.context.img_offset_x
    
    @img_offset_x.setter
    def img_offset_x(self, value):
        self.context.img_offset_x = value
    
    @property
    def img_offset_y(self):
        return self.context.img_offset_y
    
    @img_offset_y.setter
    def img_offset_y(self, value):
        self.context.img_offset_y = value
    
    @property
    def display_w(self):
        return self.context.display_w
    
    @display_w.setter
    def display_w(self, value):
        self.context.display_w = value
    
    @property
    def display_h(self):
        return self.context.display_h
    
    @display_h.setter
    def display_h(self, value):
        self.context.display_h = value
    
    @property
    def orig_w(self):
        return self.context.orig_w
    
    @orig_w.setter
    def orig_w(self, value):
        self.context.orig_w = value
    
    @property
    def orig_h(self):
        return self.context.orig_h
    
    @orig_h.setter
    def orig_h(self, value):
        self.context.orig_h = value
    ...
    def boxes(self, value):
        self.context.boxes = value
    
    @property
    def selected_idx(self):
        return self.context.selected_idx
    
    @selected_idx.setter
    def selected_idx(self, value):
        self.context.selected_idx = value
    
    @property
    def _last_auto_infer_image(self):
        return self.context.last_auto_infer_image
    
    @_last_auto_infer_image.setter
    def _last_auto_infer_image(self, value):
        self.context.last_auto_infer_image = value
    
    @property
    def drag_mode(self):
        return self.context.drag_mode
    
    @drag_mode.setter
    def drag_mode(self, value):
        self.context.drag_mode = value
    
    @property
    def dragging(self):
        return self.context.dragging
    
    @dragging.setter
    def dragging(self, value):
        self.context.dragging = value
    
    @property
    def start_x(self):
        return self.context.start_x
    
    @start_x.setter
    def start_x(self, value):
        self.context.start_x = value
    
    @property
    def start_y(self):
        return self.context.start_y
    
    @start_y.setter
    def start_y(self, value):
        self.context.start_y = value
    
    @property
    def temp_x(self):
        return self.context.temp_x
    
    @temp_x.setter
    def temp_x(self, value):
        self.context.temp_x = value
    
    @property
    def temp_y(self):
        return self.context.temp_y
    
    @temp_y.setter
    def temp_y(self, value):
        self.context.temp_y = value
    
    @property
    def move_offset_x(self):
        return self.context.move_offset_x
    
    @move_offset_x.setter
    def move_offset_x(self, value):
        self.context.move_offset_x = value
    
    @property
    def move_offset_y(self):
        return self.context.move_offset_y
    
    @move_offset_y.setter
    def move_offset_y(self, value):
        self.context.move_offset_y = value
    
    @property
    def active_handle(self):
        return self.context.active_handle
    
    @active_handle.setter
    def active_handle(self, value):
        self.context.active_handle = value
    
    @property
    def handle_size(self):
        return self.context.handle_size
    
    @handle_size.setter
    def handle_size(self, value):
        self.context.handle_size = value
    # ==================== 兼容属性层结束 ====================
    ...
    def _build_ui(self):
        ...
        self.open_project_btn = self._create_top_button(btn_wrap, get_text("BTN_OPEN_PROJECT"), self.open_project)
        self.open_project_btn.pack(side="left", padx=5, pady=12)
        self.save_btn = self._create_top_button(btn_wrap, get_text("BTN_SAVE"), self.save_current_labels)
        self.save_btn.pack(side="left", padx=5, pady=12)
        self.auto_detect_btn = self._create_top_button(btn_wrap, get_text("BTN_AUTO_DETECT"), self.not_ready_yolo)
        self.auto_detect_btn.pack(side="left", padx=5, pady=12)
        self.export_btn = self._create_top_button(btn_wrap, get_text("BTN_EXPORT"), self.export_dataset_action)
        self.export_btn.pack(side="left", padx=5, pady=12)
        self.dataset_center_btn = self._create_top_button(btn_wrap, t("TITLE_DATASET_EXPORT"), lambda: open_dataset_export_window(self.root, self.context))
        self.dataset_center_btn.pack(side="left", padx=5, pady=12)
        ...
        self.training_monitor_btn = self._create_top_button(btn_wrap, t("TOOL_TRAINING_MONITOR"), lambda: open_training_monitor(self.root))
        self.training_monitor_btn.pack(side="left", padx=5, pady=12)
        self.training_center_btn = self._create_top_button(btn_wrap, t("TOOL_TRAINING_CENTER"), lambda: open_training_center(self.root, self.config_data))
        self.training_center_btn.pack(side="left", padx=5, pady=12)
        self.import_training_config_btn = self._create_top_button(btn_wrap, t("TOOL_IMPORT_TRAINING_CONFIG"), self.import_training_config)
        self.import_training_config_btn.pack(side="left", padx=5, pady=12)
        self.import_class_config_btn = self._create_top_button(btn_wrap, t("TOOL_IMPORT_CLASS_CONFIG"), self.import_class_config)
        self.import_class_config_btn.pack(side="left", padx=5, pady=12)
        self.data_health_check_btn = self._create_top_button(btn_wrap, t("TOOL_DATA_HEALTH_CHECK"), self.open_data_health_check)
        self.data_health_check_btn.pack(side="left", padx=5, pady=12)
        self.closed_loop_btn = self._create_top_button(btn_wrap, t("TOOL_CLOSED_LOOP"), lambda: open_closed_loop_window(self.root, self.context))
        self.closed_loop_btn.pack(side="left", padx=5, pady=12)
        if not self.config_data.get("inference", {}).get("enable_openclaw", True):
            self.ai_analysis_btn.config(state="disabled", fg="#7f8a99")
        self.fit_window_btn = self._create_top_button(btn_wrap, get_text("BTN_FIT_WINDOW"), self.set_view_fit)
        self.fit_window_btn.pack(side="left", padx=5, pady=12)
        self.original_scale_btn = self._create_top_button(btn_wrap, get_text("BTN_ORIGINAL_SCALE"), self.set_view_original)
        self.original_scale_btn.pack(side="left", padx=5, pady=12)
        self.settings_btn = self._create_top_button(btn_wrap, get_text("BTN_SETTINGS"), self.show_settings_tip)
        ...
        self.project_info_label = tk.Label(
            info_card,
            text=get_text("STATUS_WAITING_PROJECT_INFO"),
            bg=self.bg_card_2,
            fg=self.text_sub,
            justify="left",
            anchor="w",
        ...
        self.class_var = tk.StringVar(value=self.current_class_names[0] if self.current_class_names else "")
        self.class_combo = ttk.Combobox(class_card, textvariable=self.class_var, values=self.current_class_names, state="readonly", width=14)
        ...
        self._create_side_button(action_card, t("BTN_COPY_PREV_LABELS"), self.copy_labels_from_previous_image).pack(fill="x", padx=12, pady=4)
        ...
        tk.Label(
            tip_card,
            text=get_text("TIPS_SHORTCUT_OPS"),
            bg=self.bg_card_2,
            fg=self.text_sub,
            justify="left",
            anchor="w",
    ...
    def get_current_class_name(self, cls_id):
        return self.context.get_class_name(cls_id)

    def get_current_class_id(self, class_name):
        return self.context.get_class_id(class_name)
    ...
    def run_openclaw_analysis(self):
        ...
        try:
            ...
            ok, result_text = analyze_with_openclaw(
                ...
                class_names=self.current_class_names,
                agent_name="main"
            )
    ...
    def run_yolo_infer(self):
        ...
        try:
            ...
            self.set_status(get_text("STATUS_YOLO_AUTO_DETECT_FAILED"))

    def get_label_path(self, image_name):
        """获取标签文件路径（兼容旧代码，调用管理器）"""
        return self.image_label_manager.get_label_path(image_name)
    ...
    def load_images(self):
        ...
        # 可以记录日志或更新状态
        if msg:
            self.set_status(f"状态：{msg}")

    def load_current_labels(self):
        """加载当前图片的标签（调用管理器）"""
        # 调用管理器加载标签
        boxes, msg = self.image_label_manager.load_current_labels()
        
        # 处理结果
        if "失败" in msg or "错误" in msg.lower():
            # 显示错误对话框
        ...
        # 可以记录状态
        if msg and not ("失败" in msg or "错误" in msg.lower()):
            self.set_status(f"状态：{msg}")

    def save_current_labels(self):
        """保存当前图片的标签（调用管理器）"""
        # 调用管理器保存标签
        success, msg = self.image_label_manager.save_current_labels()
    ...
    def maybe_autosave(self):
        """如果需要自动保存，则保存当前标签（调用管理器）"""
        # 调用管理器处理自动保存
        success, msg = self.image_label_manager.maybe_autosave()
        
        # 可以记录日志，但不显示UI提示（自动保存应该是静默的）
        if success:
            # 可选：记录自动保存日志
            pass
    ...
    def show_current_image(self):
        ...
        # 获取当前图片信息
        image_path, image_name, img_msg = self.image_label_manager.get_image_display_info()
        
        if not image_path:
            # 图片信息获取失败
            self.image_canvas.delete("all")
            self.image_canvas.create_text(300, 200, text=f"图片加载失败\n{img_msg}", fill="#ff8080", font=("Microsoft YaHei", 12))
            self.set_status(f"状态：{img_msg}")
            self.update_selected_box_panel()
            return
        ...
        # 获取画布区域尺寸
        canvas_area_width = self.canvas_area.winfo_width()
        canvas_area_height = self.canvas_area.winfo_height()
        
        # 调用管理器准备图片显示
        display_img, orig_w, orig_h, display_w, display_h, prep_msg = self.image_label_manager.prepare_image_for_display(
            image_path, canvas_area_width, canvas_area_height
        )
        
        if not display_img:
            # 图片准备失败
            self.image_canvas.delete("all")
            self.image_canvas.create_text(300, 200, text=f"图片处理失败\n{prep_msg}", fill="#ff8080", font=("Microsoft YaHei", 12))
            self.set_status(f"状态：{prep_msg}")
            self.update_selected_box_panel()
            return
        
        # 设置图片相关属性
        self.original_image = display_img  # 注意：这里存储的是调整后的图片
        self.orig_w, self.orig_h = orig_w, orig_h
        self.display_w, self.display_h = display_w, display_h
        
        # 创建Tkinter图片对象
        self.current_tk = ImageTk.PhotoImage(display_img)
        
        try:
            # UI绘制逻辑（保留在MainWindow中）
            self.image_canvas.delete("all")
            ...
            # 计算图片在画布中的偏移（居中显示）
            self.img_offset_x = max((canvas_w - display_w) // 2, 0)
            self.img_offset_y = max((canvas_h - display_h) // 2, 0)
            
            # 在画布上显示图片
            self.image_canvas.create_image(
                self.img_offset_x, self.img_offset_y, 
                anchor="nw", image=self.current_tk
            )
            ...
            self.image_info_label.config(
                text=f"{self.current_image_name}  [{current_num}/{image_count}]"
            )
            
            # 更新状态栏
            self.set_status(
                f"状态：当前图片 {current_num}/{image_count}，框数 {box_count}"
            )
    ...
    def redraw_boxes(self, temp_rect=None):
        ...
        for i, box in enumerate(self.boxes):
            ...
            label = self.get_current_class_name(box[0])
    ...
    def on_mouse_down(self, event):
        ...
        if hit_idx is not None:
            ...
            self.class_var.set(self.get_current_class_name(self.boxes[hit_idx][0]))
            self.refresh_box_list()
    ...
    def on_mouse_up(self, event):
        ...
        if self.drag_mode == "add":
            ...
            if new_box:
                ...
                self.class_var.set(self.get_current_class_name(new_box[0]))
                self.refresh_box_list()
                self.redraw_boxes()
    ...
    def copy_labels_from_previous_image(self):
        """复制上一张图片的标注到当前图片"""
        if not self.image_files:
            messagebox.showinfo("提示", "当前没有图片")
            return
        
        if self.current_index == 0:
            messagebox.showinfo("提示", "没有上一张图片")
            return
        
        # 获取上一张图片名和标签路径
        prev_image_name = self.image_files[self.current_index - 1]
        prev_label_path = self.get_label_path(prev_image_name)
        
        if not os.path.exists(prev_label_path):
            messagebox.showinfo("提示", "上一张图片没有标注")
            return
        
        # 读取上一张图片的标注
        prev_boxes = []
        try:
            with open(prev_label_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    prev_boxes.append([cls_id, cx, cy, bw, bh])
        except Exception as e:
            messagebox.showerror("错误", f"读取上一张标注失败：\n{e}")
            return
        
        if not prev_boxes:
            messagebox.showinfo("提示", "上一张图片没有标注框")
            return
        
        # 处理当前图片已有标注的情况
        if self.boxes:
            choice = messagebox.askyesnocancel("复制标注", 
                f"上一张图片有 {len(prev_boxes)} 个标注框\n"
                f"当前图片已有 {len(self.boxes)} 个标注框\n\n"
                "选择操作：\n"
                "是(Y) - 追加标注框\n"
                "否(N) - 替换当前标注框\n"
                "取消 - 放弃操作")
            ...
            if choice is None:  # 取消
                ...
                self.boxes.extend(prev_boxes)
                self.set_status(f"状态：已追加 {len(prev_boxes)} 个标注框")
            else:  # 否 - 替换
                self.boxes = prev_boxes
                self.set_status(f"状态：已替换为 {len(prev_boxes)} 个标注框")
        else:
            # 当前没有标注，直接复制
            self.boxes = prev_boxes
            self.set_status(f"状态：已复制 {len(prev_boxes)} 个标注框")
    ...
    def update_selected_box_panel(self):
        ...
        if not has_box:
            self.selected_box_title.config(text=t("LABEL_NO_BOX_SELECTED_DETAIL"))
            for field in ["cx", "cy", "w", "h"]:
        ...
        box = self.boxes[self.selected_idx]
        cls_name = self.get_current_class_name(box[0])

        self.selected_box_title.config(text=f"{t('LABEL_BOX_INDEX')}{self.selected_idx:02d}   {t('LABEL_CLASS_COLON')}{cls_name}")
    ...
    def mark_current_duplicate(self):
        import shutil
        dup_images_dir = r"./data/duplicate_cache/images"
        dup_labels_dir = r"./data/duplicate_cache/labels"
    ...
    def backup_workspace(self):
        ...
        backup_root = r"./backups"
        files_to_backup = [
            r"./main.py",
            r"./ui/main_window.py",
            r"./ui/settings_window.py",
            r"./config.json"
        ]
    ...
    def restore_workspace(self):
        ...
        restore_targets = {
            ...
            "settings_window.py": r"./ui/settings_window.py",
    ...
    def show_training_window_placeholder(self):
        ...
        monitor = open_training_monitor(self.root)

        try:
            monitor.clear_log()
            monitor.set_training_info(training_config)
            monitor.set_status("准备启动")
            monitor.set_stop_callback(lambda: self._training_runner.stop(monitor))
            monitor.set_resume_callback(lambda: self._training_runner.resume(training_config, monitor))
            monitor.append_log("[训练] 已打开训练监控窗口")
            monitor.append_log("[训练] 正在读取训练配置")
        except:
            pass

        self._training_runner.start(training_config, monitor)

    def refresh_class_selector(self):
        try:
            self.class_combo["values"] = self.current_class_names
            if self.current_class_names:
                self.class_var.set(self.current_class_names[0])
            if hasattr(self, "status_var"):
                self.status_var.set(f"当前类别源: {self.class_source_name}")
        except:
            pass
    ...
    def load_classes_from_file(self, file_path):
        ...
        if ext == ".txt":
            ...
            try:
                ...
                messagebox.showerror("导入失败", "当前环境缺少 PyYAML，请先安装 pyyaml")
                return None
    ...
    def open_data_health_check(self):
        ...
        try:
            # 确保图片已加载
            if not hasattr(self, 'image_label_manager'):
                messagebox.showwarning("提示", "请先加载图片目录")
                return
            ...
            open_data_health_window(
                ...
                on_jump_to_image=self._jump_to_image_by_name
            )
        except Exception as e:
            messagebox.showerror("打开失败", f"无法打开健康检查窗口:\n{e}")
        ...
        try:
            ...
            for i, img_name in enumerate(self.image_files):
                if img_name == image_name:
                    # 导航到该索引
                    if self.current_image_name and self.auto_save:
                        self.maybe_autosave()
            ...
            # 如果没找到精确匹配，尝试模糊匹配（不带扩展名）
            image_base = os.path.splitext(image_name)[0]
            for i, img_name in enumerate(self.image_files):
                if os.path.splitext(img_name)[0] == image_base:
                    if self.current_image_name and self.auto_save:
                        self.maybe_autosave()
            ...
            messagebox.showwarning("未找到", f"未找到图片: {image_name}")
            return False
            
        except Exception as e:
            messagebox.showerror("跳转失败", f"跳转到图片时发生错误:\n{e}")
            return False
    ...
    def import_class_config(self):
        ...
        try:
            ...
            if not classes:
                messagebox.showerror("导入失败", "未能从文件中读取有效类别")
                return

            self.current_class_names = classes
            ...
            messagebox.showinfo(
                "导入成功",
                "类别配置已导入\\n\\n"
                + f"来源: {self.class_source_name}\\n"
                + f"类别数量: {len(self.current_class_names)}\\n"
                + f"首个类别: {self.current_class_names[0]}"
            )
        except Exception as e:
            messagebox.showerror("导入失败", str(e))

    def import_training_config(self):
        file_path = filedialog.askopenfilename(
            title="导入训练配置",
            filetypes=[
                ("配置文件", "*.json;*.yaml;*.yml"),
        ...
        try:
            ...
            if ext == ".json":
                ...
                messagebox.showerror("导入失败", "不支持的配置文件格式")
                return

            if not isinstance(data, dict):
                messagebox.showerror("导入失败", "配置文件内容不是字典结构")
                return

            training_data = data.get("training", data)

            if not isinstance(training_data, dict):
                messagebox.showerror("导入失败", "未找到有效的 training 配置")
                return

            if "training" not in self.config_data or not isinstance(self.config_data.get("training"), dict):
                self.config_data["training"] = {}

            self.config_data["training"].update(training_data)

            try:
                if hasattr(self, "status_var"):
                    self.status_var.set(f"已导入训练配置: {file_path}")
            except:
                pass

            training_cfg = self.config_data["training"]
            keys = ["data", "data_yaml", "model", "epochs", "batch_size", "imgsz", "device", "project", "name"]
            summary = []
            for k in keys:
                if k in training_cfg:
                    summary.append(f"{k} = {training_cfg[k]}")

            missing = []
            if not training_cfg.get("data") and not training_cfg.get("data_yaml"):
                missing.append("data 或 data_yaml")
            if not training_cfg.get("model"):
    ...
    def on_listbox_select(self, event):
        ...
        self.class_var.set(self.get_current_class_name(self.boxes[self.selected_idx][0]))
        self.redraw_boxes()
    ...
    def change_selected_class(self, event=None):
        ...
        if new_name not in self.current_class_names:
            return
        new_id = self.get_current_class_id(new_name)
        if new_id is None:
            return
        self.boxes[self.selected_idx][0] = new_id
        self.refresh_box_list()
        self.redraw_boxes()
    ...
    def prev_image(self):
        """切换到上一张图片（调用管理器）"""
        # 调用管理器进行导航
        result = self.image_label_manager.navigate_prev()
    ...
    def next_image(self):
        """切换到下一张图片（调用管理器）"""
        # 调用管理器进行导航
        result = self.image_label_manager.navigate_next()
    ...
    def execute_action(self, action_name, **kwargs):
        ...
        返回:
            dict: {
                ...
                'data': dict or None  # 额外数据
            }
        """
        # 初始化全局分发器（如果尚未初始化）
        dispatcher = get_global_dispatcher(self)
        
        try:
            # 使用新的action system执行动作
            receipt = dispatcher.execute_action(action_name, **kwargs)
            ...
            result = {
                ...
                'data': {
                    ...
                    'preconditions_passed': receipt.preconditions_passed,
                    'preconditions_failed': receipt.preconditions_failed,
                    'changed_state': receipt.changed_state,
                    'changed_fields': receipt.changed_fields,
                    'execution_time_ms': receipt.execution_time_ms,
                    'ai_suggested': receipt.ai_suggested,
                    'ai_confidence': receipt.ai_confidence
                }
            }
            
            # 如果失败且没有错误信息，使用回执消息
            if not receipt.success and not receipt.error and receipt.message:
                result['error'] = receipt.message
            
            return result
            
        except Exception as e:
            # 如果新的action system失败，回退到旧的动作映射
            import traceback
            error_details = f"{type(e).__name__}: {str(e)}\n{traceback.format_exc()}"
            
            # 记录错误
            print(f"Action system failed for {action_name}, falling back: {error_details}")
            
            # 回退到旧的动作映射（保持向后兼容）
            return self._execute_action_fallback(action_name, **kwargs)

    def _execute_action_fallback(self, action_name, **kwargs):
        """
        旧版动作映射回退（保持向后兼容）
        ...
        返回:
            dict: 兼容格式的执行结果
        """
        # 旧版动作映射表：action_name -> (method, success_message, needs_kwargs)
        action_map = {
            # 基础导航动作
            'prev_image': (
                self.prev_image,
            ...
            'copy_labels_from_previous_image': (
                self.copy_labels_from_previous_image,
                '已复制上一张图片标注',
                False
            ),
            'run_openclaw_analysis': (
                self.run_openclaw_analysis,
                '已启动 OpenClaw 分析',
                False
            ),
            
            # OpenClaw分身能力扩张v2：无弹窗选框控制
            'select_next_box': (
            ...
            'select_prev_box': (
                self.select_prev_box,
                '已选中上一个框',
                False
            ),
            'select_box_by_index': (
                self.select_box_by_index,
                '已按索引选中框',
                True  # 需要box_index参数
            ),
            'delete_box_by_index': (
                self.delete_box_by_index,
                '已按索引删除框',
                True  # 需要box_index参数
            ),
            'analyze_box_by_index': (
                self.analyze_box_by_index,
                '已按索引分析框',
                True  # 需要box_index参数
            ),
            'edit_box_by_index': (
                self.edit_box_by_index,
                '已按索引打开编辑窗口',
                True  # 需要box_index参数
            ),
            'change_box_class_by_index': (
                self.change_box_class_by_index,
                '已按索引修改框类别',
                True  # 需要box_index和class_name参数
            ),
            
            # OpenClaw分身能力扩张v2第二刀：无弹窗框微调动作
            'move_box_by_index': (
                self.move_box_by_index,
                '已按索引移动框',
                True  # 需要box_index、dx、dy参数
            ),
            'resize_box_by_index': (
                self.resize_box_by_index,
                '已按索引调整框大小',
                True  # 需要box_index、dw、dh参数
            ),
            'nudge_box_by_index': (
                self.nudge_box_by_index,
                '已按索引微调框位置',
                True  # 需要box_index、direction参数，可选amount
            ),
            'set_box_coords_by_index': (
                self.set_box_coords_by_index,
                '已按索引设置框坐标',
                True  # 需要box_index，可选cx、cy、w、h参数
            ),
            
            # OpenClaw分身能力扩张v2：标注批处理增强冲刺包
            'change_all_boxes_class': (
                self.change_all_boxes_class,
                '已批量修改全部框类别',
                True  # 需要target_class参数
            ),
            'change_boxes_class_by_source_class': (
                self.change_boxes_class_by_source_class,
                '已按源类别批量修改框类别',
                True  # 需要source_class和target_class参数
            ),
            'delete_boxes_by_class': (
                self.delete_boxes_by_class,
                '已按类别批量删除框',
                True  # 需要class_name参数
            ),
            'clear_current_image_boxes': (
                self.clear_current_image_boxes,
                '已清空当前图全部标注框',
                False
            ),
            'analyze_all_boxes': (
                self.analyze_all_boxes,
                '已分析全部标注框',
                False
            ),
            'analyze_boxes_by_class': (
                self.analyze_boxes_by_class,
                '已按类别分析标注框',
                True  # 需要class_name参数
            ),
        }
        ...
        if action_name not in action_map:
            return {
                ...
                'error': f'未知动作名称，支持的动作: {", ".join(action_map.keys())}'
            }
        
        # 获取对应的方法、成功消息和参数需求
        method, success_message, needs_kwargs = action_map[action_name]
                ...
                if isinstance(result, dict):
                    ...
                    if result.get('success', False):
                        ...
                        return {
                            'success': False,
                            'action': action_name,
                            'message': result.get('message', f'执行动作失败: {action_name}'),
                            'error': result.get('error', '方法返回失败')
                        }
                elif result:
                    # 返回布尔值的动作
                    return {
                    ...
                    return {
                        'success': False,
                        'action': action_name,
                        'message': f'执行动作失败: {action_name}',
                        'error': '方法返回False，可能参数无效或条件不满足'
                    }
            else:
                # 不需要参数的动作
                result = method()
    ...
    def get_boss_status_summary(self):
        ...
        """
        # 更新背包中的运行时数据
        if 'runtime_data' in self.backpack:
            self.backpack['runtime_data']['last_action'] = self.backpack['runtime_data'].get('last_action')
            self.backpack['runtime_data']['action_count'] = self.backpack['runtime_data'].get('action_count', 0)
        
        # 构建状态摘要
        summary = {
            'role': '老板 (MainWindow)',
            'current_image_name': self.current_image_name or '无',
            'current_image_index': self.current_index if hasattr(self, 'current_index') else -1,
            ...
            'selected_box_index': self.selected_idx if hasattr(self, 'selected_idx') else None,
            'class_names_count': len(self.current_class_names) if hasattr(self, 'current_class_names') else 0,
            'backpack_data': {
                'tools_count': len(self.backpack.get('tools', {})),
                'resources_count': len(self.backpack.get('resources', {})),
                'notes_count': len(self.backpack.get('notes', [])),
                'runtime_data': self.backpack.get('runtime_data', {})
            },
            'recent_actions': {
                'last_action': self.backpack.get('runtime_data', {}).get('last_action'),
                'action_count': self.backpack.get('runtime_data', {}).get('action_count', 0),
                'start_time': self.backpack.get('runtime_data', {}).get('start_time')
            },
            'ui_state': {
                'has_image_dir': bool(self.image_dir),
                'has_label_dir': bool(self.label_dir),
                'auto_save_enabled': self.auto_save if hasattr(self, 'auto_save') else False,
                'alpha_value': self.alpha_value if hasattr(self, 'alpha_value') else 1.0
            },
    ...
    def get_mainboard_health_summary(self):
        ...
        # 冲刺目标D：主板健康摘要增强
        health_summary = {
            'mainboard': 'MainWindow 主板',
            'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            
            # 冲刺目标A：状态统一收口检查
            'context_health': {
                'context_available': hasattr(self, 'context') and self.context is not None,
                'context_type': type(self.context).__name__ if hasattr(self, 'context') else 'None',
                'state_sources_unified': self._check_state_sources_unified(),
            },
            
            # 冲刺目标B：类别真值源收口检查
            'class_source_health': {
                'current_class_names_available': hasattr(self, 'current_class_names') and self.current_class_names is not None,
                'class_names_count': len(self.current_class_names) if hasattr(self, 'current_class_names') else 0,
                'class_source_unified': self._check_class_source_unified(),
                'has_legacy_class_names': hasattr(self, 'CLASS_NAMES') and self.CLASS_NAMES is not None,
            },
            
            # 冲刺目标C：动作入口统一收口检查
            'action_entry_health': {
                'execute_action_available': hasattr(self, 'execute_action'),
                'action_count_in_backpack': self.backpack.get('runtime_data', {}).get('action_count', 0),
                'action_entries_unified': self._check_action_entries_unified(),
            },
            
            # 主板插槽状态
            'mainboard_slots': {
                'organization_slot': self.organization is not None,
                'dispatcher_slot': self.dispatcher is not None,
                'assistant_roles_slot': len(self.assistant_roles) > 0,
                'reporter_slot': self.reporter is not None,
                'release_manager_slot': self.release_manager is not None,
            },
            ...
            'backpack_health': {
                ...
                'resources_count': len(self.backpack.get('resources', {})),
                'notes_count': len(self.backpack.get('notes', [])),
                'runtime_data_available': 'runtime_data' in self.backpack,
            },
            
            # UI状态同步检查
            'ui_sync_health': {
                'has_canvas_panel': self.canvas_panel is not None,
                'has_right_panel': self.right_panel is not None,
                'refresh_methods_available': all(hasattr(self, method) for method in ['refresh_box_list', 'redraw_boxes', 'update_selected_box_panel']),
            },
    ...
    def _check_state_sources_unified(self):
        """检查状态源是否统一"""
        # 检查关键状态是否通过context统一
        state_checks = []
        
        # 检查current_class_names是否通过property装饰器统一
        if hasattr(self, 'current_class_names'):
            state_checks.append('current_class_names unified via property')
        
        # 检查context是否包含关键状态
        if hasattr(self, 'context') and self.context is not None:
            context_attrs = ['current_class_names', 'boxes', 'selected_idx', 'current_image_name']
            for attr in context_attrs:
                if hasattr(self.context, attr):
                    state_checks.append(f'context.{attr} available')
        
        return len(state_checks) > 5  # 如果有足够多的状态检查通过，则认为状态源基本统一
    
    def _check_class_source_unified(self):
        """检查类别源是否统一"""
        # 检查是否使用current_class_names作为主要类别源
        checks = []
        
        # 检查property装饰器是否存在
        if hasattr(type(self), 'current_class_names') and isinstance(type(self).current_class_names, property):
            checks.append('property decorator exists')
        
        # 检查context中的current_class_names是否可用
        if hasattr(self, 'context') and hasattr(self.context, 'current_class_names'):
            checks.append('context.current_class_names available')
        
        # 检查是否仍然有直接使用CLASS_NAMES的地方
        if hasattr(self, 'CLASS_NAMES') and self.CLASS_NAMES is not None:
            checks.append('legacy CLASS_NAMES exists (for compatibility)')
        
        return len(checks) >= 2  # 至少property和context都可用
    
    def _check_action_entries_unified(self):
        """检查动作入口是否统一"""
        checks = []
        
        # 检查execute_action方法是否存在
        if hasattr(self, 'execute_action'):
            checks.append('execute_action method exists')
        
        # 检查action_map是否存在
        if hasattr(self, 'execute_action'):
            # 通过检查execute_action方法中是否有action_map来间接判断
            checks.append('action entry mapping likely exists')
        
        # 检查背包中的动作计数
        if self.backpack.get('runtime_data', {}).get('action_count', 0) > 0:
            checks.append('actions have been executed')
    ...
    def _calculate_health_score(self):
        ...
        # 状态统一收口（30分）
        if self._check_state_sources_unified():
            score += 30
        
        # 类别真值源收口（30分）
        if self._check_class_source_unified():
            score += 30
        
        # 动作入口统一收口（20分）
        if self._check_action_entries_unified():
            score += 20
        ...
        slot_count = sum([
            ...
            len(self.assistant_roles) > 0,
            self.reporter is not None,
            self.release_manager is not None
        ])
        score += min(10, slot_count * 2)  # 每个插槽2分，最多10分
    ...
    def _generate_health_recommendations(self):
        ...
        # 基于健康检查生成建议
        if not self._check_state_sources_unified():
            recommendations.append('继续推进状态统一收口，减少双份状态风险')
        
        if not self._check_class_source_unified():
            recommendations.append('确保所有类别读取都通过current_class_names property')
        
        if not self._check_action_entries_unified():
            recommendations.append('统一动作入口，避免同一动作多入口多实现')
        
        # 检查主板插槽
        slot_checks = [
            ('organization', '组织架构插槽'),
            ('dispatcher', '调度器插槽'),
            ('assistant_roles', '助手角色插槽'),
            ('reporter', '报告器插槽'),
            ('release_manager', '发布管理器插槽')
        ]
        
        for attr, name in slot_checks:
            if getattr(self, attr) is None:
                recommendations.append(f'考虑实现{name}以分担主板职责')
        
        if not recommendations:
            recommendations.append('主板状态健康，可继续推进框架化建设')
    ...
    def select_next_box(self):
        ...
        if self.selected_idx is None:
            ...
            # 选中下一个框，循环到开头
            new_idx = (self.selected_idx + 1) % len(self.boxes)
        
        return self._select_box_by_index(new_idx)
    ...
    def select_box_by_index(self, box_index):
        """
        按索引选中框
        
        参数:
            box_index: 框的索引（0-based）
        
        返回:
            bool: 是否成功选中
        """
        return self._select_box_by_index(box_index)
    ...
    def delete_box_by_index(self, box_index):
        ...
        if not (0 <= box_index < len(self.boxes)):
            self.set_status(f"状态：无效的框索引 {box_index}，有效范围 0-{len(self.boxes)-1}")
            return False
        ...
        self.refresh_box_list()
        self.redraw_boxes()
        self.set_status(f"状态：已删除框 #{box_index}（类别: {cls_name}），当前剩余 {len(self.boxes)} 个框")
        
        return True

    def analyze_box_by_index(self, box_index):
        """
        按索引分析框（调用OpenClaw分析）
        ...
        # 先选中该框
        if not self._select_box_by_index(box_index):
            return False
        
        # 调用OpenClaw分析选中框
        try:
            # 使用现有的run_openclaw_analysis方法
            self.run_openclaw_analysis()
            return True
        except Exception as e:
            self.set_status(f"状态：分析框 #{box_index} 时出错: {str(e)}")
            return False

    def edit_box_by_index(self, box_index):
        """
        按索引编辑框（打开编辑窗口）
        ...
        # 打开编辑窗口（简化版，实际应打开编辑窗口）
        try:
            # 这里应该打开编辑窗口，目前先设置为选中状态并返回成功
            self.set_status(f"状态：已选中框 #{box_index}，准备编辑（编辑窗口功能待实现）")
            return True
        except Exception as e:
            self.set_status(f"状态：打开框 #{box_index} 编辑窗口时出错: {str(e)}")
            return False

    def change_box_class_by_index(self, box_index, class_name):
        """
        按索引修改框类别
        ...
        if class_name not in self.current_class_names:
            self.set_status(f"状态：无效的类别名称 '{class_name}'，有效类别: {', '.join(self.current_class_names)}")
            return False
        ...
        try:
            # 获取类别ID
            class_id = self.get_current_class_id(class_name)
            if class_id is None:
                self.set_status(f"状态：无法找到类别 '{class_name}' 的ID")
                return False
            
            # 更新框的类别
            self.boxes[box_index][0] = class_id
            
            # 刷新UI
            self.refresh_box_list()
            self.redraw_boxes()
            self.set_status(f"状态：已修改框 #{box_index} 类别为 {class_name}")
            
            return True
        except Exception as e:
            self.set_status(f"状态：修改框 #{box_index} 类别时出错: {str(e)}")
            return False

    def _select_box_by_index(self, box_index):
        """
        内部方法：按索引选中框并刷新UI
        ...
        box = self.boxes[box_index]
        cls_name = self.get_current_class_name(box[0])
        self.set_status(f"状态：已选中框 #{box_index}（类别: {cls_name}）")
        
        return True

    # ==================== OpenClaw分身能力扩张v2第二刀：无弹窗框微调动作 ====================

    def move_box_by_index(self, box_index, dx, dy):
        """
        按索引移动框（相对移动）
        ...
        self.set_status(f"状态：已移动框 #{box_index} (dx={dx:.3f}, dy={dy:.3f})")
        return True

    def resize_box_by_index(self, box_index, dw, dh):
        """
        按索引调整框大小（相对调整）
        ...
        self.set_status(f"状态：已调整框 #{box_index} 大小 (dw={dw:.3f}, dh={dh:.3f})")
        return True

    def nudge_box_by_index(self, box_index, direction, amount=0.01):
        """
        按索引微调框位置（小步移动）
        ...
        if direction not in direction_map:
            self.set_status(f"状态：无效的移动方向 '{direction}'，有效方向: {', '.join(direction_map.keys())}")
            return False
        ...
        self.set_status(f"状态：已微调框 #{box_index} 向{direction}移动 {amount:.3f}")
        return True

    def set_box_coords_by_index(self, box_index, cx=None, cy=None, w=None, h=None):
        """
        按索引设置框坐标（绝对设置）
        ...
        if changes:
            self.set_status(f"状态：已设置框 #{box_index} 坐标 ({', '.join(changes)})")
        else:
            self.set_status(f"状态：框 #{box_index} 坐标未变化")
    ...
    # 冲刺目标A：批量改类
    def change_all_boxes_class(self, target_class):
        """
        批量改类：将当前图全部框统一改成目标类别
        ...
        # 获取目标类别ID
        target_class_id = self.get_current_class_id(target_class)
        if target_class_id is None:
            return {
                'success': False,
                'changed_count': 0,
                'message': f'无效的类别名称: {target_class}'
            }
        ...
        for i in range(len(self.boxes)):
            if self.boxes[i][0] != target_class_id:
                self.boxes[i][0] = target_class_id
                changed_count += 1
        ...
        message = f'已将 {changed_count} 个框修改为 {target_class}' if changed_count > 0 else '所有框已是目标类别'
        self.set_status(f"状态：{message}")
    ...
    def change_boxes_class_by_source_class(self, source_class, target_class):
        """
        批量改类：将指定源类别的框批量改成目标类别
        ...
        # 获取类别ID
        source_class_id = self.get_current_class_id(source_class)
        target_class_id = self.get_current_class_id(target_class)
        ...
        if source_class_id is None:
            return {
                'success': False,
                'changed_count': 0,
                'message': f'无效的源类别名称: {source_class}'
            }
        ...
        if target_class_id is None:
            return {
                'success': False,
                'changed_count': 0,
                'message': f'无效的目标类别名称: {target_class}'
            }
        ...
        # 批量修改
        for i in range(len(self.boxes)):
            if self.boxes[i][0] == source_class_id:
                self.boxes[i][0] = target_class_id
                changed_count += 1
        ...
        message = f'已将 {changed_count} 个 {source_class} 框修改为 {target_class}' if changed_count > 0 else f'未找到 {source_class} 类别的框'
        self.set_status(f"状态：{message}")
    ...
    # 冲刺目标B：批量删除
    def delete_boxes_by_class(self, class_name):
        """
        批量删除：删除当前图中指定类别的全部框
        ...
        message = f'已删除 {deleted_count} 个 {class_name} 类别的框' if deleted_count > 0 else f'未找到 {class_name} 类别的框'
        self.set_status(f"状态：{message}")
    ...
    def clear_current_image_boxes(self):
        """
        批量删除：清空当前图全部标注框
        ...
        message = f'已清空 {deleted_count} 个标注框'
        self.set_status(f"状态：{message}")
    ...
    # 冲刺目标C：批量分析入口
    def analyze_all_boxes(self):
        """
        批量分析：对当前图全部框发起统一分析入口
        
        返回:
            dict: {
                'success': bool,      # 是否成功
                'analyzed_count': int, # 分析的框数量
                'message': str        # 结果消息
            }
        """
        if not self.boxes:
        ...
        # 调用OpenClaw分析（复用现有分析链路）
        try:
            # 使用现有的run_openclaw_analysis方法
            self.run_openclaw_analysis()
            
            message = f'正在分析全部 {len(self.boxes)} 个标注框'
            self.set_status(f"状态：{message}")
            
            return {
                'success': True,
                'analyzed_count': len(self.boxes),
                'message': message
            }
        except Exception as e:
            error_message = f'分析全部框时出错: {str(e)}'
    ...
    def analyze_boxes_by_class(self, class_name):
        """
        批量分析：只分析当前图中某个类别的框
        ...
        try:
            ...
            message = f'正在分析 {class_box_count} 个 {class_name} 类别的框'
            self.set_status(f"状态：{message}")
            
            return {
                'success': True,
                'analyzed_count': class_box_count,
                'message': message
            }
        except Exception as e:
            error_message = f'分析 {class_name} 类别框时出错: {str(e)}'
            self.set_status(f"状态：{error_message}")
